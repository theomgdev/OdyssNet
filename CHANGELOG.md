...
- The brake's loss-EWMA/variance now use a bias-correction-style warmup (`alpha = max(brake_ema_alpha, 1/(step+2))`, the same technique already used for Adam's own bias correction elsewhere in this file): the first ~20 calls after construction (or after a checkpoint reload) average over everything seen so far instead of committing to the steady-state window immediately, so the variance estimate isn't built from a handful of noisy early samples.
