...
import numpy as np
import torch
from odyssnet import OdyssNet, OdyssNetTrainer, TrainingHistory, save_checkpoint, transplant_weights, set_seed
...
def main():
    ...
    for t_loss, s_loss in zip(transfer_losses, scratch_losses):
        history.record(transplanted_loss=t_loss, scratch_loss=s_loss)
    history.plot(title="Skill Transfer: Multiply Convergence (Transplanted vs Scratch)")
