...
*   **Novelty Gate:** Before accumulation, each raw correlation matrix is element-wise scaled by $g_{ji} = 1/(1+|W_{\text{eff},ji}|)$. Strong existing weights produce expected co-activations (tautological echo), not genuine learning. The gate suppresses these redundant updates and amplifies plasticity on weak, unexplored connections. It is parameter-free and detached — no second-order gradients through the weight matrix.
