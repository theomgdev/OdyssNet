...
*   **The step count is a dial:** on a fixed grid it is not one — the weights are fitted to a single cadence, and fidelity falls from 97.2% at 6 steps to 68.8% at 64. Drawing the step count per batch (`--k-range`, now the default at 12–20) cuts that spread from 42.8 points to 14.8 on one seed and 45.4 to 10.4 on another, while holding the Frechet distance flat. Trained over 12–20, it holds at K=4 and K=64 either side: what it learns is that the cadence is a quantity to read, not the range it saw. See `--mode flex`.
