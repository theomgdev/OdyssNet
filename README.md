...
*   **The step count as a dial:** because the training grid is the sampling grid, the weights are fitted to one cadence — fidelity falls from 97.2% at 6 steps to 68.8% at 64. `--k-range 12,20` draws the step count per batch and cuts that spread from 42.8 points to 14.8 while holding the Frechet distance flat; `--cadence` tells each frame the stride it is about to take, lifting fidelity at every step count and costing sample distribution. Both are off by default. See `--mode flex`.
