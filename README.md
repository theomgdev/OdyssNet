...
*   **Hebbian Plasticity:** Optional online Hebbian learning (`use_hebbian=True`) — the network accumulates temporal neuron correlations and learns *how fast to learn* via two fully differentiable logit parameters (`hebb_factor`, `hebb_decay`).
...
*   **Hebbian Plasticity**: When `use_hebbian=True`, temporal correlations $h_t \otimes h_{t-1}$ are accumulated each step and injected into the effective weight matrix as $W_{\text{eff}} = W + \text{hebb\_lr} \cdot C_t$. The learning rate and decay are themselves learnable, letting the network adapt how quickly its synaptic weights track activity patterns.
