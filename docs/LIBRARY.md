...
Four properties define it, each one the answer to something the earlier implementation got wrong.

*   **A normalized, zero-initialized gain (`hebb_norm`, +N parameters).** The trace is RMS-normalized and scaled by a learnable gain before it is added to `W`. Previously the strength factor multiplied twice — once accumulating into the trace, once applying it — so the effective gain was `sigmoid(logit)²`, roughly 32x below the measured optimum while the logit claimed otherwise, and nothing could move it. Now the factor decides *which* synapses are plastic and the gain decides *how much*. Because the gain starts at zero, turning plasticity on changes the network by exactly nothing until training decides otherwise: `hebb_type` is an ablation with one variable in it, the same property temporal attention gets from its zero-init output projection.
*   **A differentiable correlation.** Two stop-gradients used to sit on this path: one hiding the correlation's dependence on the state, one truncating the trace's own history. Only the second costs memory and only the first costs capability, so the first is gone and the second stays (`local.detach()` in the update). Measured on episodic recall, that recovers three quarters of the gap for the same bytes (0.8321 → 0.9550).
*   **A novelty gate.** Co-activation across an already-strong synapse is tautological — the neuron fired because the weight drove it, not because the pattern is new — so the correlation is damped by `1 / (1 + |W_eff|)`. Parameter-free and detached, so it shapes plasticity without opening a second-order path through `W`.
*   **A per-example trace.** The live trace is `(B, N, N)`: every example writes its own associations. With one trace shared across a batch the per-example associations average away and a batched forward computes a different function than the same examples run one at a time (measured: 9.5e-3 against 2.98e-7) — training then never exercises the mechanism inference uses, which is why plasticity could not do one-shot binding at all. The *persistent* buffer stays `(N, N)` and holds the batch mean, so checkpoints, neurogenesis padding and weight transplants are untouched.

The cost is real and worth stating plainly: every step retains a `(B, N, N)` trace for the backward pass, so memory grows as `steps x B x N²`. That is affordable exactly where plasticity earns its place — small cores with short rollouts — and unaffordable on a large core, where it was measured to contribute nothing anyway. Run big cores with `hebb_type=None`.

**When plasticity pays.** The repair makes the mechanism work; it does not make it universally worth switching on, and the measurements disagree with each other by task. On sequential patch classification with attention off, plasticity was ahead (1.0088 / 87.99% at 480 parameters against 1.0548 / 87.41% at 430, epoch 31, one seed). On the *same* task with four attention heads held fixed it was behind on both seeds tried, by +0.02 of loss and -0.5 points of accuracy for +50 parameters. On single-injection classification it was slightly behind. Read that as the rule stated above — plasticity is for tasks where step *T* extends what step *T-1* built — plus a caution that attention seems to fill the same role on the record task and to fill it better. Ablate it rather than assuming; `hebb_type` is now a one-variable switch precisely so that ablation is cheap and exact.

**Use `torch.compile`.** Plasticity is bound by kernel launches, not arithmetic: profiled on a 10-neuron core at batch 32 over 16 steps, a step issues thousands of launches for about 20 ms of GPU work. Eager, `hebb_type='both'` adds 132% to the step; compiled it adds 26% (13.8 → 32.1 ms eager, 6.4 → 8.1 ms compiled). Temporal and spatial are evaluated on one stacked path axis so `'both'` costs one set of kernels rather than two.
...
**The largest speedup available is `torch.compile`, and attention stays out of its graph so it can be had.** Fusing the echo loop is worth 2.2x on a bare core, 4.0x with plasticity and 3.6x with plasticity and attention together (10 neurons, batch 32, 16 steps, RTX 3060 Ti) — which is the whole reason plasticity is affordable at all: eager it adds 132% to the step, compiled it adds 26%. But `torch.compile` miscompiles the fp32 region above: Inductor's dtype model reports float for a buffer it emits as half, and `o_proj`'s matmul dies on `Half != float` (an explicit `.float()` does not help — the same wrong model folds it away). `attend` and `write` therefore carry `@torch._dynamo.disable`.

What the break costs depends on what is left to fuse around it. With plasticity on there is a great deal, and it is free: 17.4 ms/batch with `hebb_type='both'` and four heads, against 17.6 for a fully traced half-precision path and 60.4 eager. **With attention as the only addition to a bare core there is not, and compiling buys close to nothing** — 48.5 ms/batch against 45.4 eager on the record config. So `torch.compile` is worth reaching for when plasticity, a wider core or a longer rollout gives the step body some substance, and worth measuring rather than assuming when attention is the only thing switched on.

The decorator stays regardless, on both counts. Removing it breaks every compiled run with attention on. And half-precision attention — the only way to trace the path — is *worse*, not merely different: on embedded MNIST with four heads it trailed the fp32 path by a stable ~0.017 of loss from the fourth epoch on (0.8220 against 0.8043 at epoch 8, same seed and same initial weights). Trading that for a speedup that does not arrive would be the wrong bargain twice.
