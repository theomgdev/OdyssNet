...
That costs nothing measurable, and it is not a compromise between speed and accuracy. The step is launch-bound and the rest of it still fuses around the break — 17.4 ms/batch with plasticity and four heads, against 17.6 for a fully traced half-precision path and 60.4 eager. Half-precision attention is also *worse*, not merely different: on embedded MNIST with four heads it trailed the fp32 path by a stable ~0.017 of loss from the fourth epoch on (0.8220 against 0.8043 at epoch 8, same seed and same initial weights). Keep the decorator; removing it breaks every compiled run with attention on.
